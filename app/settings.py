...
from app.auxiliary import sortArrayWithArray, getTimestamp

# this class reads and writes application settings
class AppSettings():
    def __init__(self):
        # check if settings file exists and creates it if it doesn't
        if not os.path.exists('./netspyre.conf'):
            print('[+] Creating settings file..')
            self.createDefaultSettings()
        else:
            print('[+] Loading settings file..')
            self.actions = QtCore.QSettings('./netspyre.conf', QtCore.QSettings.NativeFormat)
    ...
    def createDefaultSettings(self):
        ...
        self.actions.setValue('default-terminal','xterm')
...
#       self.actions.setValue("nbtscan","netbios-ns")
    ...
    def backupAndSave(self, newSettings):
        ...
        os.rename('./netspyre.conf', './'+getTimestamp()+'-netspyre.conf')  
        self.actions = QtCore.QSettings('./netspyre.conf', QtCore.QSettings.NativeFormat)
